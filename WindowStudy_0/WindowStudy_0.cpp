// WindowStudy_0.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//

#include "framework.h"
#include "WindowStudy_0.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_WINDOWSTUDY0, szWindowClass, MAX_LOADSTRING);
    WORD rst = MyRegisterClass(hInstance);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINDOWSTUDY0));
    ...
    // Wany
    // 메세지 큐에서 하나 씩 꺼내온다
    // GetMesage의 단점: 메세지가 없으면 반환 안함.
    // 게임에서는 GetMessage 대신 PeekMessage 많이 씀.
    // PeekMessage는 메세지 큐에 메세지가 없어도 반환 됨.
    ...
    //      render();
    //  }
    // 이런 식으로 사용하여 메세지 있으면 메세지 처리하고, 없으면 연산처리 한다.
