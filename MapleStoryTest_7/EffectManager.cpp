...
bool EffectManager::Load(std::wstring _path)
{
	...
	for (auto& file : std::filesystem::directory_iterator(path))
	{
		...
		if (file.path().extension() == L"")
		{
			...
			if (filename == L"Damage")
			{
				LoadDamageEffect(dir);
			}
			else
			{
...
bool EffectManager::LoadDamageEffect(std::wstring _path)
{
	std::filesystem::path path(_path);
	for (auto& file : std::filesystem::directory_iterator(path))
	{
		...
		{
			std::wstring dir = filepath + L"/";
			auto it = damageEffectList.find(dir);
			if (it != damageEffectList.end())
			{
				continue;
			}

			std::unique_ptr<DamageEffect> newEffect = std::make_unique<DamageEffect>();
			if (newEffect->Load(dir))
			{
				newEffect->initialize();
				damageEffectList.insert(std::make_pair(filename, std::move(newEffect)));
			}
		}
		else
		{
...
bool EffectManager::addDamageEffectToJobList(Vector2f _pos, std::wstring _name, float _damage)
{
	auto it = damageEffectList.find(_name);
	if (it != damageEffectList.end())
	{
		DamageEffect* pEffect = new DamageEffect(it->second.get());
		//pEffect = it->second.get();
		pEffect->initialize();
		pEffect->setCamera(renderCamera);
		pEffect->setPos(_pos);
		pEffect->setDamage(static_cast<int>(_damage));
		//pEffect->moveCenterTo(_pos);
		jobList.push_back(pEffect);
		return true;
	}
