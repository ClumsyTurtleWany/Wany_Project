...
public:
	bool create(int _column, int _row, std::vector<Vertex>& _vertexList);
	node3D<float>* createNode(int _LT, int _RT, int _LB, int _RB, node3D<float>* _parent = nullptr);
	bool buildTree(node3D<float>* _parent);
