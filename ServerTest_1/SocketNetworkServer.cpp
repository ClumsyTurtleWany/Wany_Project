...
bool SocketNetwork::Server::SendMsgToHost(std::string name, std::string& msg, int length)
{
	if (msg.empty())
	{
		return false;
	...
	return SocketNetwork::Send(*it->second, msg, length);
}

bool SocketNetwork::Server::SendMsgToClient(std::string name, std::string& msg, int length)
{
	if (!isOpen)
	{
		return false;
...
bool SocketNetwork::Server::SendMsgToHostAll(std::string& msg, int length)
{
	if (msg.empty())
	{
		return false;
	...
	for (auto& it : HostList)
	{
		...
		SocketNetwork::Send(*Socket, msg, length);
	}

	return true;
}

bool SocketNetwork::Server::SendMsgToClientAll(std::string& msg, int length)
{
	if (!isOpen)
	{
		return false;
...
bool SocketNetwork::Server::SendMsgToAll(std::string& msg, int length)
{
	if (msg.empty())
	{
		return false;
	...
	bool isSucessed = SendMsgToHostAll(msg, length);

	if (isOpen)
	{
		isSucessed &= SendMsgToClientAll(msg, length);
	}
...
DWORD __stdcall SocketNetwork::Server::ReceiveThread(LPVOID lpParam)
{
	...
	while (Client != nullptr)
	{
		...
		int length = 256;
		if (SocketNetwork::Receive(*Client, ReceiveMsg, length))
		{
			if (ReceiveMsg.empty())
			{
				delete Client;
