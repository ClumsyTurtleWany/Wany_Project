...
void Core::initialize()
{
	...
	if (user == nullptr)
	{
		...
		user->setTexture(DXTextureManager::getInstance()->getTexture(Texture_Key::Player));
		//user->initialize();
	}
	...
	for (int i = 0; i < 5; i++)
	{
		...
		obj->setTexture(DXTextureManager::getInstance()->getTexture(Texture_Key::Npc));
		//obj->initialize();
		map->addObject(obj);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		obj->createShader();
		//obj->setDevice(m_pd3dDevice, m_pImmediateContext);
		obj->setTexture(DXTextureManager::getInstance()->getTexture(Texture_Key::Obstacle));
		//obj->initialize();
		map->addObject(obj);
