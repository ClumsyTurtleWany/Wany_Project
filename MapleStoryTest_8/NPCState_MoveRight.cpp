...
bool NPCState_MoveRight::frame()
{
	...
	if (effectTime >= nextFrame)
	{
		...
		{
			npc->currentDirection = NPC::Direction::Right;
			npc->flipSprite(true);
...
bool NPCState_MoveRight::render()
{
	//std::wstring strUserState;
	//strUserState += L"UserState: ";
	//strUserState += L"NPCState_MoveRight";
	//DXWriter::getInstance()->draw(0, 100, strUserState);

	return true;
}

bool NPCState_MoveRight::release()
{
	return true;
}